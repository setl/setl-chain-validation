 ...
 */
public class TestAssetClassUpdateTx {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    AssetClassUpdateTx tx = new AssetClassUpdateTx( 1,            //  chainId
        2,            //  int1
        hash,
        3,            //  nonce
        true,
    ...
    assertTrue(hash.equals("f8b9334e378ac8dcdbf3688a91227c18bc735e24a1b1e6f4e1e4cb9c300441c5"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.UPDATE_ASSET_CLASS));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.UPDATE_ASSET_CLASS));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();
    
    AssetClassUpdateTx tx2 = AssetClassUpdateTx.decodeTX( new MPWrappedArrayImpl( encoded));
    AssetClassUpdateTx tx3 = new AssetClassUpdateTx(tx);
    AssetClassUpdateTx tx4 = (AssetClassUpdateTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(AssetClassUpdateTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (AssetClassUpdateTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.UPDATE_ASSET_CLASS.getPriority());
