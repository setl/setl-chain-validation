...
public class PoaLockAssetTxTest {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    PoaLockAssetTx tx = new PoaLockAssetTx(1, 2, hash, 3, true, fromPubKey, fromAddress, poaAddress, reference, nameSpace,
        classId, lockDetails,protocol,  meta, signature, 5, poa, 6);
    
    hash = Hex.encode(SHA256.sha256(tx.buildHash(new JSONHashAccumulator()).getBytes()));
    assertTrue(hash.equals("b6f23048ee966f79e5110b38f0e9a0a5ae3336118519164bc15acb63693c6eee"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.POA_LOCK_ASSET));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.POA_LOCK_ASSET));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();
    PoaLockAssetTx tx2 = PoaLockAssetTx.decodeTX(encoded);
    PoaLockAssetTx tx3 = new PoaLockAssetTx(tx);
    PoaLockAssetTx tx4 = (PoaLockAssetTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(PoaLockAssetTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (PoaLockAssetTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.POA_LOCK_ASSET.getPriority());
