 ...
 */
public class TestAssetClassDeleteTx {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    AssetClassDeleteTx tx = new AssetClassDeleteTx( 1,            //  chainId
        2,            //  int1
        hash,
        3,            //  nonce
        true,
    ...
    assertTrue(hash.equals("fe1f9ccda243cf5da04b62f4bc546ea30b0f449bab2660b9ed14b736c233d75d"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.DELETE_ASSET_CLASS));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.DELETE_ASSET_CLASS));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();
    
    AssetClassDeleteTx tx2 = AssetClassDeleteTx.decodeTX( new MPWrappedArrayImpl( encoded));
    AssetClassDeleteTx tx3 = new AssetClassDeleteTx(tx);
    AssetClassDeleteTx tx4 = (AssetClassDeleteTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(AssetClassDeleteTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (AssetClassDeleteTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.DELETE_ASSET_CLASS.getPriority());
