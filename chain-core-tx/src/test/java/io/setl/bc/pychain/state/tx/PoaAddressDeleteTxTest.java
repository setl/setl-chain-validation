...
public class PoaAddressDeleteTxTest {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    PoaAddressDeleteTx tx = new PoaAddressDeleteTx( 1,            //  chainId
        2,            //  int1
        hash,
        3,            //  nonce
        true,
    ...
    assertTrue(hash.equals("e5e6e04a7381385ee5dbde6152bfad0ee5ff1bd24b300feb53770f40224944bb"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.POA_DELETE_ADDRESS));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.POA_DELETE_ADDRESS));
    assertTrue(((AddressDeleteTx)tx).getTxType().equals(TxType.POA_DELETE_ADDRESS));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();

    PoaAddressDeleteTx tx2 = PoaAddressDeleteTx.decodeTX( new MPWrappedArrayImpl( encoded));
    PoaAddressDeleteTx tx3 = new PoaAddressDeleteTx(tx);
    PoaAddressDeleteTx tx4 = (PoaAddressDeleteTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(PoaAddressDeleteTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (PoaAddressDeleteTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.POA_DELETE_ADDRESS.getPriority());
