...
public class TestAssetTransferTx {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    AssetTransferTx tx = new AssetTransferTx( 1,            //  chainId
                                              2,            //  int1
                                              hash,
                                              3,            //  nonce
                                              true,         //  updated
    ...
    assertTrue(hash.equals("1fb6d3fcba0f7cf90439c57bacc0f11a39ca638792022300921d9a7ee6fa3f1f"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.TRANSFER_ASSET));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.TRANSFER_ASSET));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();

    AssetTransferTx tx2 = AssetTransferTx.decodeTX( new MPWrappedArrayImpl( encoded));
    AssetTransferTx tx3 = new AssetTransferTx(tx);
    AssetTransferTx tx4 = (AssetTransferTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(AssetTransferTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (AssetTransferTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.TRANSFER_ASSET.getPriority());
