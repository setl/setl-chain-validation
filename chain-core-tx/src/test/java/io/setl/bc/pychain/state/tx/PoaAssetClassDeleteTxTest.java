...
public class PoaAssetClassDeleteTxTest {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    PoaAssetClassDeleteTx tx = new PoaAssetClassDeleteTx(
        1,
        4,
        hash,
        3,
    ...
    assertTrue(hash.equals("18dd9193c247f1384a3f56a70fc07d52e8e3e5ae55e1fead141f44907cc1bf66"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.POA_DELETE_ASSET_CLASS));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.POA_DELETE_ASSET_CLASS));
    assertTrue(((AssetClassDeleteTx)tx).getTxType().equals(TxType.POA_DELETE_ASSET_CLASS));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();
    PoaAssetClassDeleteTx tx2 = PoaAssetClassDeleteTx.decodeTX(encoded);
    PoaAssetClassDeleteTx tx3 = new PoaAssetClassDeleteTx(tx);
    PoaAssetClassDeleteTx tx4 = (PoaAssetClassDeleteTx) TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(PoaAssetClassDeleteTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (PoaAssetClassDeleteTx thisTx : Arrays.asList(new PoaAssetClassDeleteTx[] {tx, tx2, tx3, tx4})) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.POA_DELETE_ASSET_CLASS.getPriority());
