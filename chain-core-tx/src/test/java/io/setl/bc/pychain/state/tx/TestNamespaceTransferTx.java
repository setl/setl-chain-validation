...
public class TestNamespaceTransferTx {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    NamespaceTransferTx tx = new NamespaceTransferTx( 1,            //  chainId
        2,            //  int1
        hash,
        3,            //  nonce
        true,
    ...
    assertTrue(hash.equals("e2ecf4316cad4d22b22463125637baf83cc7b576d07ac1273a35c6a008dbe077"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.TRANSFER_NAMESPACE));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.TRANSFER_NAMESPACE));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();
  
    NamespaceTransferTx tx2 = NamespaceTransferTx.decodeTX( new MPWrappedArrayImpl( encoded));
    NamespaceTransferTx tx3 = new NamespaceTransferTx(tx);
    NamespaceTransferTx tx4 = (NamespaceTransferTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(NamespaceTransferTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (NamespaceTransferTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.TRANSFER_NAMESPACE.getPriority());
