...
public class PoaAssetIssueEncumberTxTest {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    PoaAssetIssueEncumberTx tx = new PoaAssetIssueEncumberTx(1, hash, 3, true, fromPubKey, fromAddress, poaAddress, poaReference, nameSpace,
    ...
    assertTrue(hash.equals("4f4b5c93a867012211c7eea89c529abda3470e9d0bf6affb928bd427b5eee1e1"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.POA_ISSUE_AND_ENCUMBER_ASSET));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.POA_ISSUE_AND_ENCUMBER_ASSET));
    assertTrue(!tx.getTxType().equals(TxType.ISSUE_AND_ENCUMBER_ASSET));

    Object[] encoded = tx.encodeTx();
    PoaAssetIssueEncumberTx tx2 = PoaAssetIssueEncumberTx.decodeTX(encoded);
    PoaAssetIssueEncumberTx tx3 = new PoaAssetIssueEncumberTx(tx);
    PoaAssetIssueEncumberTx tx4 = (PoaAssetIssueEncumberTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(PoaAssetIssueEncumberTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (PoaAssetIssueEncumberTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.POA_ISSUE_AND_ENCUMBER_ASSET.getPriority());
