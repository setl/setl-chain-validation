 ...
 */
public class TestTransferFromManyTx {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    TransferFromManyTx tx = new TransferFromManyTx(
        1,            //  chainId
        hash,
        3,            //  nonce
        true,
    ...
    assertTrue(hash.equals("a3828534ad6df83ea3fa74b475c971100525468c9df060fdd5de7ef95e8e73f7"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.TRANSFER_ASSET_FROM_MANY));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.TRANSFER_ASSET_FROM_MANY));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();

    TransferFromManyTx tx2 = TransferFromManyTx.decodeTX( new MPWrappedArrayImpl( encoded));
    TransferFromManyTx tx3 = new TransferFromManyTx(tx);
    TransferFromManyTx tx4 = (TransferFromManyTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(TransferFromManyTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (TransferFromManyTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.TRANSFER_ASSET_FROM_MANY.getPriority());
