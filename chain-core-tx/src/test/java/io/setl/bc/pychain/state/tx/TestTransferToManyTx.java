 ...
 */
public class TestTransferToManyTx {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    TransferToManyTx tx = new TransferToManyTx( 1,            //  chainId
                                                hash,
                                                3,            //  nonce
                                                true,
                                                fromPubKey,
    ...
    assertTrue(hash.equals("3ff7200a44f1d11806c3d81ea3c0cc20cebf06ac496596acfd9b7a258af35abf"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.TRANSFER_ASSET_TO_MANY));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.TRANSFER_ASSET_TO_MANY));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();

    TransferToManyTx tx2 = TransferToManyTx.decodeTX( new MPWrappedArrayImpl( encoded));
    TransferToManyTx tx3 = new TransferToManyTx(tx);
    TransferToManyTx tx4 = (TransferToManyTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(TransferToManyTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (TransferToManyTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.TRANSFER_ASSET_TO_MANY.getPriority());
