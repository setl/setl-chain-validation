 ...
 */
public class PoaTransferToManyTxTest {
  ...
  public void testSerialiseDeserialise() throws Exception {
    ...
    final Object[] toAddress  = new Object[] {};
    ...
    PoaTransferToManyTx tx = new PoaTransferToManyTx( 1,            //  chainId
        hash,
        3,            //  nonce
        true,
        fromPubKey,
    ...
    assertTrue(hash.equals("f56810763ded7af3f8d9d1b0f6ce9dadbc34e5fe09b03ff2fb95170d7129429d"));
    tx.setHash(hash);

    assertTrue(tx.getTxType().equals(TxType.POA_TRANSFER_ASSET_TO_MANY));
    assertTrue(((AbstractTx)tx).getTxType().equals(TxType.POA_TRANSFER_ASSET_TO_MANY));
    assertTrue(!tx.getTxType().equals(TxType.ADD_X_CHAIN));

    Object[] encoded = tx.encodeTx();
    
    PoaTransferToManyTx tx2 = PoaTransferToManyTx.decodeTX( new MPWrappedArrayImpl( encoded));
    PoaTransferToManyTx tx3 = new PoaTransferToManyTx(tx);
    PoaTransferToManyTx tx4 = (PoaTransferToManyTx)TxFromList.txFromList(new MPWrappedArrayImpl(encoded));

    Object[] encodedBad = tx.encodeTx();
    encodedBad[TxGeneralFields.TX_TXTYPE] = -1;
    assertNull(PoaTransferToManyTx.decodeTX( new MPWrappedArrayImpl( encodedBad)));

    tx.toString();  // Just for code coverage. This method is just for display purposes at the moment.

    for (PoaTransferToManyTx thisTx : Arrays.asList(tx, tx2, tx3, tx4)) {

      assertTrue(thisTx.addresses().containsAll(tx.addresses()));
      assertTrue(thisTx.addresses().size() == tx.addresses().size());
      assertTrue(thisTx.getPriority() == TxType.POA_TRANSFER_ASSET_TO_MANY.getPriority());
