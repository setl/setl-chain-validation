...
import io.setl.bc.pychain.tx.create.TransferFromMany;
...
public class TransferFromManyTest extends BaseTestClass {
  ...
  public void updatestate() throws Exception {
    ...
    // issueAmount to sAddress1, sAddress2, sAddress3
    ...
    thisTX = TransferFromMany
        .transferFromManyUnsigned(chainID, addressNonce++, poaPubKey, attorneyAddress, namespace, classname, subjectAddresses,
    ...
    thisTX = TransferFromMany
        .transferFromManyUnsigned(chainID, addressNonce, poaPubKey, "Dross", namespace, classname, subjectAddresses,
            transferAmount, protocol, metadata, poa);
    ...
    thisTX = TransferFromMany
        .transferFromManyUnsigned(chainID, addressNonce, poaPubKey, poaAddress, namespace, classname, subjectAddresses,
    ...
    thisTX = TransferFromMany.transferFromManyUnsigned(
        chainID,
        addressNonce++,
        poaPubKey, poaAddress,
        namespace,
        classname,
    ...
    // Check Balance pre bad transfer
    assertEquals(assetBalances.find(sAddress1).getClassBalance().getOrDefault(fullAssetID, BALANCE_ZERO), issueAmount);

    assertFalse(UpdateState.update(thisTX, s0, thisTX.getTimestamp(), thisTX.getPriority(), false));

    // Check Balance post bad transfer, should not have changed.
    assertEquals(assetBalances.find(sAddress1).getClassBalance().getOrDefault(fullAssetID, BALANCE_ZERO), issueAmount);
    ...
    // Now test that Balance entries are tidied up when balances got to Zero.

    subjectAddresses = new Object[]{
        new Object[]{sAddress1, (issueAmount - transferAmount)},
        new Object[]{sAddress2, (issueAmount - transferAmount)},
        new Object[]{sAddress3, (issueAmount - transferAmount)}};

    thisTX = TransferFromMany
        .transferFromManyUnsigned(chainID, addressNonce++, poaPubKey, poaAddress, namespace, classname,
