  ...
  public static ReturnTuple updateEvent(StateSnapshot stateSnapshot, long updateTime, String contractAddress, EventData eventData, boolean checkOnly) {
    ...
    if (contractEntry == null) {
      ...
      return new ReturnTuple((checkOnly ? SuccessType.WARNING : SuccessType.PASS),
          MessageFormat.format("TokensNominate updateEvent() contractEntry == null, eventType = {0}. Address : {1}",
              eventData.getEventName(), contractAddress));
    }

    TokensNominateContractData contractData = (TokensNominateContractData) contractEntry.getContractData();

    if (contractData == null) {
      // This should not be possible. contractEntry.getContractData() will likely throw an error rather than return null.
    ...
    if (!CONTRACT_NAME_TOKENS_NOMINATE.equalsIgnoreCase(contractData.get__function())) {
      // This should not be possible. contractEntry.getContractData() instantiates the contract data based on it's function name.
    ...
    switch (eventData.getEventName().toLowerCase()) {
      ...
      case "time":
        ...
        if (referenceTime >= contractData.get__timeevent()) {
          ...
          if (referenceTime >= contractData.expiry) {
            // Expiry
          ...
          // The Tokens_nominate contract should only have one time event, at expiry. Odd that this code should get run at all.
