var JSONTree = (function() {
    ...
    var _jsObj = function(object, depth, indent) {
        ...
        if(!indent && depth == 0){
            ...
            var body = [
                _openBracket('{', indent ? depth : 0, id),
                _span(content, {id: id, class: 'jstHiddenBlock'}),
                _closeBracket('}', depth)
            ].join('\n');
            return _span(body, {class: 'jstFolded'})
        }
    };
    ...
    var _jsArr = function(array, depth, indent) {
        ...
        if(!indent && depth == 0){
            ...
            var arr = [
                _openBracket('[', indent ? depth : 0, id),
                _span(body, {id: id, class: 'jstHiddenBlock'}),
                _closeBracket(']', depth)
            ].join('\n');
            return _span(arr, {class: 'jstFolded'});
        }
    };
    ...
    var _property = function(name, value, depth) {
        ...
        return _span(property + propertyValue, {class: 'jstProperty ' + name});
    }
    ...
    var _openBracket = function(symbol, depth, id ) {
        var local_id = id.split("_")[1];// ID within the instance.
        if(depth == 0 && local_id == 0) {
            return (
            ...
            return (
            _span(_indent(symbol, depth), {class: 'jstBracket'}) +
            _span('', {class: 'jstExpand', onclick: 'JSONTree.toggle(\'' + id + '\')'})
            );
        }
    }
